...
- `internal/providers/ths`：同花顺当日/历史涨停池；支持 page/limit/date/filter/order 参数，默认查询当前交易日、按最后封板时间倒序，接口返回 `status_code != 0` 时按 `UPSTREAM_ERROR` 错误码处理，并为同花顺 provider 配置浏览器式默认请求头。
